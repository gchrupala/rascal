...
class Batcher(object):
    ...
    def __init__(self, mapper, pad_end=False, visual=True):
        autoassign(locals())
    ...
    def batch(self, gr):
        ...
        simembed = numpy.array([ x['simembed'] for x in gr], dtype='float32') if gr[0]['simembed'] is not None else None
        #simmatrix = numpy.zeros((len(gr), len(gr)), dtype='float32')
        #for i in range(len(gr)):
        #    #for j in range(max(i, 0), len(gr)): #FIXME avoid 
        #    for j in range(0, len(gr)):
        #        simmatrix[i, j] = stringsim(gr[i]['rawtext'], gr[j]['rawtext'])
        return { 'input': inp,
...
class SimpleData(object):
    ...
    def __init__(self, provider, tokenize=words, min_df=10, scale=True, scale_input=False, scale_utt=False,
        ...
        self.batcher = Batcher(self.mapper, pad_end=False, visual=visual)
        ...
        for sent in image['sentences']:
            ...
            if speakers is None or speaker in speakers:
                ...
                yield {'tokens_in':  toks,
                       'tokens_out': toks,
                       'rawtext': sent.get('raw'),
                       'audio':       sent.get('audio'),
                       'img':        image.get('feat'),
                       'speaker':   speaker,
                       'simembed': sent.get('simembed')
                        }
