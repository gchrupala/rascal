...
class SpeechCorrText(nn.Module):
...
def experiment(net, data, run_config):
    ...
    with open("result.json", "w") as out:
      for epoch in range(last_epoch+1, run_config['epochs'] + 1):
        costs = dict(SpeechText=Counter(), SpeechCorrText=Counter(), SpeechImage=Counter(), TextImage=Counter())
        ...
        for _j, items in enumerate(zip(data['SpeechImage'].iter_train_batches(reshuffle=True), 
                                       ...
                                       data['SpeechCorrText'].iter_train_batches(reshuffle=True),
                                       data['TextImage'].iter_train_batches(reshuffle=True))):
            j = _j + 1  
            item = dict(SpeechImage=items[0], SpeechText=items[1], SpeechCorrText=items[2], TextImage=items[3])
